...
public class SATSolver {
	...
	public static void main(String[] args) throws IOException {
		...
		//String input = "D:\\Dropbox\\shitty courses\\561 new\\HW2P\\input2.txt";
		//String output = "D:\\Dropbox\\shitty courses\\561 new\\HW2P\\output2.txt";
		//double p = 0.7;
		//int max_flips = 100;
	...
	*/
	...
	public static int[][] randomflip(int[][] S, boolean[] sentence, int M, int N) {
		...
		for (int i=0;i<N;i++) {
			for (int j=0;j<M;j++) {
				if (sentence[i*M+j]==true || sentence[M*N+i*M+j]==true) {
					involved.add(i*M+j);
					System.out.println("	involved: i="+i+",j="+j);
				}
			}
		}
		double random = Math.random();
		...
		System.out.println("	before change:"); printout3(S);
		S[j][i] = (int) (1-S[j][i]);
		System.out.println("	after change:"); printout3(S);
		return S;
	}
